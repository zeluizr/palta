...
import * as currency from '../../src/br/currency.js'

describe('br/currency', () => {
  describe('format', () => {
    it('formata valor simples', () => {
      expect(currency.format(1234.56)).toBe('R$ 1.234,56')
    })
    it('formata zero', () => {
      expect(currency.format(0)).toBe('R$ 0,00')
    })
    it('formata negativo', () => {
      expect(currency.format(-1234.56)).toBe('R$ -1.234,56')
    })
    it('formata milhões', () => {
      expect(currency.format(1234567.89)).toBe('R$ 1.234.567,89')
    })
    it('formata sem símbolo', () => {
      expect(currency.format(1234.56, { symbol: false })).toBe('1.234,56')
    })
    it('formata sem decimais', () => {
      expect(currency.format(1234, { decimals: 0 })).toBe('R$ 1.234')
    })
    it('formata valor inteiro', () => {
      expect(currency.format(15990)).toBe('R$ 15.990,00')
    })
  })

  describe('parse', () => {
    it('faz parse de valor formatado', () => {
      expect(currency.parse('R$ 1.234,56')).toBe(1234.56)
    })
    it('faz parse de zero', () => {
      expect(currency.parse('R$ 0,00')).toBe(0)
    })
